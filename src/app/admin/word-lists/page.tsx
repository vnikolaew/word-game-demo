...
export default function WordListsPage() {
  ...
  const handleCreateWordList = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "فشل في إنشاء قائمة الكلمات"
      );
    }
  };
  ...
  const handleUpdateWordList = async (id: number) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "فشل في تحديث قائمة الكلمات"
      );
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {selectedWordList ? "تعديل قائمة الكلمات" : "إضافة قائمة كلمات"}
            </DialogTitle>
          </DialogHeader>
                ...
                {words.map((word, index) => (
                  <div key={index} className="flex gap-2 items-center w-full">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        updateWord(index, "isNonWord", !word.isNonWord)
                      }
                      className={word.isNonWord ? "bg-gray-200" : ""}
                    >
