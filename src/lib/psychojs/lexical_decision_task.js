...
import { core, data, sound, util, visual, hardware } from "./src/index.js";
const { PsychoJS } = core;
...
psychoJS.openWindow({
   fullscr: true,
   color: new util.Color([0.0902, -1.0, -1.0]),
   units: "height",
   waitBlanking: true,
   backgroundImage: "",
   backgroundFit: "none",
...
psychoJS.start({
   ...
   resources: [
      // resources:
      { name: "experimental_trials.xlsx", path: "experimental_trials.xlsx" },
   ],
});
   ...
   return async function () {
      ...
      if (t >= 0.0 && key_resp_1.status === PsychoJS.Status.NOT_STARTED) {
         ...
         psychoJS.window.callOnFlip(function () {
            key_resp_1.clock.reset();
         }); // t=0 on next screen flip
         psychoJS.window.callOnFlip(function () {
            key_resp_1.start();
         }); // start on screen flip
         psychoJS.window.callOnFlip(function () {
            key_resp_1.clearEvents();
         });
      }
      ...
      if (key_resp_1.status === PsychoJS.Status.STARTED) {
         ...
         if (_key_resp_1_allKeys.length > 0) {
            key_resp_1.keys =
               _key_resp_1_allKeys[_key_resp_1_allKeys.length - 1].name; // just the last key pressed
            key_resp_1.rt =
               _key_resp_1_allKeys[_key_resp_1_allKeys.length - 1].rt;
            key_resp_1.duration =
               _key_resp_1_allKeys[_key_resp_1_allKeys.length - 1].duration;
...
function InstructionsRoutineEnd(snapshot) {
   return async function () {
      ...
      if (typeof key_resp_1.keys !== "undefined") {
         // we had a response
...
function TrialRoutineBegin(snapshot) {
   return async function () {
      ...
      _response_allKeys = [];
      trial_counter.setText(
         (trials.thisN + 1).toString() + "/" + trials.nTotal.toString()
      );
      // keep track of which components have finished
      TrialComponents = [];
   ...
   return async function () {
      ...
      if (response.status === PsychoJS.Status.STARTED) {
         ...
         if (_response_allKeys.length > 0) {
            response.keys =
               _response_allKeys[_response_allKeys.length - 1].name; // just the last key pressed
            response.rt = _response_allKeys[_response_allKeys.length - 1].rt;
            response.duration =
               _response_allKeys[_response_allKeys.length - 1].duration;
            // was this correct?
...
function FeedbackRoutineBegin(snapshot) {
   return async function () {
      ...
      if (corrAns === "left") {
         non_word_rts.push(response.rt);
      } else {
         word_rts.push(response.rt);
      }

      fbtxt.setText(fb);
      trial_counter_2.setText(
         (trials.thisN + 1).toString() + "/" + trials.nTotal.toString()
      );
      // keep track of which components have finished
      FeedbackComponents = [];
...
function FeedbackRoutineEachFrame() {
   return async function () {
      ...
      frameRemains = 0.0 + 1 - psychoJS.window.monitorFramePeriod * 0.75; // most of one frame period left
      if (
         trial_counter_2.status === PsychoJS.Status.STARTED &&
         t >= frameRemains
      ) {
...
function EndRoutineBegin(snapshot) {
   return async function () {
      ...
      word_rt_av = Number.parseInt(util.average(word_rts) * 1000);
      non_word_av = Number.parseInt(util.average(non_word_rts) * 1000);

      psychoJS.experiment.addData("End.started", globalClock.getTime());
      thank_you.setText(
         "You scored " +
            correct_counter.toString() +
            "/" +
            trials.nTotal.toString() +
            " correct!\nAverage response time for words was: " +
            word_rt_av.toString() +
            "ms\nAverage response time for non words was: " +
            non_word_av.toString() +
            "ms"
      );
      // keep track of which components have finished
      EndComponents = [];
